...
function AnalysisPage({ord,sOrd,execs,scenarios,toast,getKey}){
  ...
  if(o) return(<><button className="bt bs bm" onClick={()=>sSel(null)} style={{marginBottom:8}}>← Назад</button>
    <div className="cd"><div className="hd"><h3>🔍 Анализ: {o.id}</h3><span className="bg">{o.clientName} · {SM[o.stage]?.l}</span></div>
      ...
      <div className="fg"><label className="fl">Исполнитель</label><select className="fsl" value={o.assignedTo||""} onChange={e=>{const u={...o,assignedTo:e.target.value};sOrd(p=>p.map(x=>x.id===o.id?u:x));toast("Исполнитель: "+e.target.value,"success")}}><option value="">Не назначен</option>{execs.map(ex=><option key={ex.id} value={ex.name}>{ex.name} ({ex.role==="translator"?"Перев.":"Ред."})</option>)}</select></div>
      <div className="fg"><label className="fl">Промпт перевода</label><textarea className="fi" rows={3} value={o.customPrompt||PR[o.docType]||""} onChange={e=>{const u={...o,customPrompt:e.target.value};sOrd(p=>p.map(x=>x.id===o.id?u:x))}} style={{resize:"vertical",fontFamily:"'JetBrains Mono',monospace",fontSize:9}}/></div>
      <div className="fg"><label className="fl">ИИ для этого заказа</label><div>{AI_PROVIDERS.map(ai=><span key={ai.id} className={`ai-chip ${(o.aiProvider||"gemini")===ai.id?"on":""}`} onClick={()=>{const u={...o,aiProvider:ai.id};sOrd(p=>p.map(x=>x.id===o.id?u:x));toast(ai.name+" выбран","success")}}>{ai.icon} {ai.name}{getKey(ai.id)?"":"  ⚠"}</span>)}</div></div>
...
function TranslPage({ord,sOrd,ak,aok,md,gas,gok,toast,getKey}){
  ...
  const run=async(o)=>{
    ...
    try{
      ...
      const u={...o,stage:next,aiTranslation:t,editedTranslation:t};
      sOrd(p=>p.map(x=>x.id===o.id?u:x));
      if(gok&&gas){try{await gas.sf(o.id,"ai_translation",t,o.langPair,o.id+"_AI.txt");await gas.us(o.id,next==="ai_edit"?"Ред. ИИ":"На ревью");}catch{}}
...
function EditorPage({ord,sOrd,ak,aok,md,aep,gas,gok,toast,getKey}){
  ...
  const run=async(o)=>{
    ...
    try{
      ...
      if(t){const u={...o,stage:"review",editedTranslation:t};sOrd(pp=>pp.map(x=>x.id===o.id?u:x));
...
function NewOrd({cur,sCur,ord,sOrd,ak,aok,md,gas,gok,aep,execs,toast,getKey}){
  ...
  const uploadAndExtract=async()=>{if(!aok)return;sUpl(true);sPrg(10);
    if(gok&&gas){for(const doc of df){try{await gas.sf("PENDING","original",doc.text||"["+doc.file.name+"]",lp,doc.file.name);}catch{}}}
    let all="";sPrg(30);
    ...
    sPrg(50);if(all.trim()){sSt(all.trim().substring(0,50000));toast("✓ "+all.trim().split(/\s+/).length+" слов","success");}sUpl(false);};
...
function RevPage({ord,sOrd,gas,gok,ak,aok,md,aep,toast}){
  ...
  if(a){const[f,t]=a.langPair.split("-");return<><button className="bt bs bm" onClick={()=>sAid(null)} style={{marginBottom:8}}>←</button>
