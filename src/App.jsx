  ...
  return(<><style>{CSS}</style><div className="R">
    ...
    <main className="M">
      ...
      <div className="CT">
        ...
        {pg==="analysis"&&<AnalysisPage {...{ord,sOrd,execs,scenarios,toast,getKey}}/>}
...
// ═══ ANALYSIS PAGE ═══
function AnalysisPage({ord,sOrd,execs,scenarios,toast,getKey}){
  ...
  const runAnalysis=async()=>{if(!o)return;const key=getKey(o.aiProvider||"gemini");if(!key){toast("Нет ключа","error");return;}
    sRunning(true);
    try{const t=await callAI(key,o.aiModel||"gemini-2.0-flash",(o.analysisPrompt||defAnalysisPrompt)+"\n\n"+o.sourceText?.substring(0,5000),null,o.aiProvider||"gemini");
    const u={...o,stage:"translate",analysisResult:t,customPrompt:o.customPrompt||PR[o.docType]||""};sOrd(p=>p.map(x=>x.id===o.id?u:x));toast("🔍 Анализ → Перевод","success");
    }catch(e){toast(e.message,"error");}sRunning(false);};
...
function NewOrd({cur,sCur,ord,sOrd,ak,aok,md,gas,gok,aep,execs,toast,getKey}){
  ...
  const uploadAndExtract=async()=>{if(!aok)return;sUpl(true);sPrg(10);
    const fileLinks=[];
    // Upload originals to Drive 01_Originals
    const oid="TW-"+String(ord.length+1).padStart(4,"0");
    if(gok&&gas){for(let i=0;i<df.length;i++){const doc=df[i];sPrg(10+Math.round(i/df.length*20));
      ...
      else if(doc.text){const r=await gas.sf(oid,"original",doc.text,lp,doc.file.name);fileLinks.push({name:doc.file.name,url:r.fileUrl,stage:"original"});}}catch(e){toast("Drive: "+e.message,"error");}}}
    // Extract text via AI
    let all="";sPrg(35);
    for(const doc of df){if(doc.text)all+=doc.text+"\n\n";else if(doc.b64){try{all+=(await callAI(ak,md,"Извлеки ВЕСЬ текст. Только текст.",[{mime:doc.mime,b64:doc.b64}]))+"\n\n";}catch(e){toast(doc.file.name+": ошибка","error");}}}
    sPrg(50);
    if(all.trim()){sSt(all.trim().substring(0,50000));}
    // Create order and go to Analysis
    const srcT=all.trim()||mn;const wc=srcT?srcT.split(/\s+/).length:0;
    ...
    sPrg(60);toast("📁 Файлы на Drive → Анализ","success");sUpl(false);};
  ...
  if(!res) return(<>
    ...
    <div className="cd"><div className="hd"><h3>📄 Файлы</h3><span className="bg">Шаг 2</span></div>
      ...
      <div className="bts">{hasFiles&&!st&&<button className="bt bp" disabled={upl||!aok} onClick={uploadAndExtract}>{upl?<><span className="spn"/> ...</>:"🚀 Запустить"}</button>}{(st||mn.trim())&&dt&&<button className="bt bv" disabled={trl} onClick={translate}>{trl?<><span className="spn"/></>:"🤖 Перевести"}</button>}</div>
