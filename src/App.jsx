...
export default function App(){
  ...
  return(<><style>{CSS}</style><div className="R">
    ...
    <main className="M">
      ...
      <div className="CT">
        {pg==="new"&&<NewOrd {...{cur,sCur,ord,sOrd,ak,aok,md,gas:gas.current,gok,aep,execs,toast,getKey}}/>}
...
function AnalysisPage({ord,sOrd,execs,scenarios,toast,getKey,gas,gok}){
  ...
  const runAnalysis=async()=>{if(!o)return;const key=getKey(o.aiProvider||"gemini");if(!key){toast("Нет ключа","error");return;}
    sRunning(true);sRunMsg("Анализ...");sRunProg(10);
    try{
      // Step 1: Extract text from files via AI
      let srcText=o.sourceText||"";
      if(!srcText&&o.fileLinks?.length){sRunMsg("Извлечение текста из файлов...");sRunProg(15);
        // We need file content — re-read from the stored b64 if available, or just use what we have
      }
      const t=await callAI(key,o.aiModel||"gemini-2.0-flash",(o.analysisPrompt||defAnalysisPrompt)+"\n\nТекст документа:\n"+(srcText||"[Файлы загружены на Drive, анализируй по метаданным: "+o.fileName+", тип: "+(o.docType||"не определён")+", клиент: "+o.clientName+"]"),null,o.aiProvider||"gemini");
      sRunProg(30);const u={...o,analysisResult:t};sOrd(p=>p.map(x=>x.id===o.id?u:x));toast("🔍 Анализ завершён","success");
  ...
  const runFullPipeline=async(order)=>{
    ...
    try{
      // Step 1: Extract text from original files if needed
      if(!srcText&&order.fileLinks?.length){sRunMsg("Извлечение текста из "+order.fileLinks.length+" файл(ов)...");sRunProg(10);
        // For files on Drive we need to re-extract — but we may not have b64 anymore
        // Use the file names as context, prompt AI about them
        srcText="Документы: "+order.fileName+". Тип: "+(order.docType||"")+". Клиент: "+order.clientName;
      }
      // Step 2: Translate
...
function NewOrd({cur,sCur,ord,sOrd,ak,aok,md,gas,gok,aep,execs,toast,getKey}){
