...
const SM={intake:{l:"Принят",i:"📥"},classify:{l:"Анализ",i:"🔍"},translate:{l:"AI Перевод",i:"🤖"},ai_edit:{l:"Ред. ИИ",i:"🔧"},review:{l:"Ревью",i:"✏️"},deliver:{l:"Готов",i:"✅"},archive:{l:"Архив",i:"📦"}};
...
class FileManager{
  ...
  async findOrCreateFolder(name,parentId){
    ...
    const c=await this.api("https://www.googleapis.com/drive/v3/files",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name,mimeType:"application/vnd.google-apps.folder",parents:parentId?[parentId]:[]})});
    return c.id;
  }
  ...
  async uploadFile(file,folderId,fileName){
    ...
    const r=await fetch("https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id,name,webViewLink",{method:"POST",headers:{"Authorization":"Bearer "+this.token},body:bodyBlob});
    return r.json();
  }
  async uploadText(text,folderId,fileName,mimeType="text/plain"){
    const metadata={name:fileName,parents:[folderId]};
    const b="-------314159265358979323846";
    const body=`\r\n--${b}\r\nContent-Type: application/json; charset=UTF-8\r\n\r\n${JSON.stringify(metadata)}\r\n--${b}\r\nContent-Type: ${mimeType}; charset=UTF-8\r\n\r\n${text}\r\n--${b}--\r\n`;
    const r=await fetch("https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id,name,webViewLink",{method:"POST",headers:{"Authorization":"Bearer "+this.token,"Content-Type":`multipart/related; boundary=${b}`},body});
    return r.json();
  }
...
export default function App(){
  ...
  const connectDrive=async()=>{
    ...
    if(!document.getElementById("gis-script")){
      ...
      s.onload=async()=>{const ok=await fm.current.init();sDriveOk(ok);if(ok)toast("Drive подключён","success");else toast("Drive: отменено","error");};
  ...
  const handleDelete = async (id) => {
    if(!window.confirm("Точно удалить заказ навсегда? Это не восстановить!")) return;
    sOrd(p => p.filter(o => o.id !== id));
    if(gok && gas.current) {
      try { await gas.current.rm(id); toast("Заказ удалён", "info"); } 
      catch(e) { toast("Ошибка таблицы: "+e.message, "error"); }
    }
  };
  ...
  return(<><style>{CSS}</style><div className="R">
    ...
    <aside className="S"><div className="Sh"><h1>The Words</h1><p>Translation Bureau</p></div>
      ...
      <div className="Sf"><div className="sfr"><span className="l">Drive</span><span className="v">{driveOk?"✓ OAuth":"✗"}</span></div><div className="sfr"><span className="l">Sheets</span><span className="v">{gok?"✓ POST":"✗"}</span></div></div>
    </aside>
    <main className="M">
      <div className="TB">
...
function AnalysisPage({ord,sOrd,execs,toast,getKey,gas,gok,fm,driveOk,md,onGoPage,handleArchive,handleDelete}){
  ...
  const runFullPipeline=async()=>{
    ...
    try{
      ...
      if(!translSaved&&!driveOk)translSaved=true;
      if(!translSaved){toast("⚠ Перевод не сохранён","error");sRunning(false);return;}
  ...
  return(<>{active.map(o=><div key={o.id} className="cd" style={{cursor:"pointer"}} onClick={()=>sSel(o.id)}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}><div>
      ...
      <div style={{fontSize:10,color:"var(--t3)"}}>{DT.find(d=>d.id===o.docType)?.icon} {o.langPair?.toUpperCase().replace("-"," → ")} · <span className={`stg ${o.stage==="translate"?"str":o.stage==="ai_edit"?"sae":o.stage==="review"?"srv":"si"}`}>{SM[o.stage]?.l}</span></div>
    </div>
...
// ═══ TRANSLATION PAGE ═══
function TranslPage({ord,handleArchive,handleDelete}){
  const withTranslation=ord.filter(o=>o.aiTranslation && o.stage!=="archive");
  ...
  return(<>{withTranslation.map(o=>{const tFiles=(o.fileLinks||[]).filter(f=>f.stage==="ai_translation");
    return<div key={o.id} className="cd">
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        ...
        <div style={{display:"flex",gap:4}}>
           <button className="bt bs bm" onClick={()=>handleArchive(o.id)} title="В архив">📦</button>
           <button className="bt bs bm" onClick={()=>handleDelete(o.id)} style={{color:"var(--er)"}} title="Удалить">🗑</button>
        </div>
      </div>
...
// ═══ EDITOR PAGE ═══
function EditorPage({ord,handleArchive,handleDelete}){
  const edited=ord.filter(o=>(o.editedTranslation&&o.editedTranslation!==o.aiTranslation||o.editorNotes) && o.stage!=="archive");
...
// ═══ ORDERS ═══
function OrdPage({ord,handleArchive,handleDelete}){
  if(!ord.length)return<div className="em"><div className="ei">📋</div><p>Пусто</p></div>;
  return(<div className="cd" style={{padding:0,overflow:"auto"}}><table className="ot"><thead><tr><th>ID</th><th>Дата</th><th>Клиент</th><th>Языки</th><th>Тип</th><th>₸</th><th>Статус</th><th>Действия</th></tr></thead>
    <tbody>{ord.map(o=><tr key={o.id}>
      <td style={{fontFamily:"'JetBrains Mono',monospace",color:"var(--ac)"}}>{o.id}</td>
      <td style={{color:"var(--t3)"}}>{new Date(o.createdAt).toLocaleDateString()}</td>
      <td>{o.clientName}</td>
      <td>{o.langPair?.toUpperCase().replace("-"," → ")}</td>
      <td>{DT.find(d=>d.id===o.docType)?.label||""}</td>
      <td style={{fontSize:9}}>{o.price?o.price+"₸":"—"}</td>
      <td><span className={`stg ${o.stage==="deliver"?"sdl":o.stage==="review"?"srv":o.stage==="archive"?"sar":"si"}`}>{SM[o.stage]?.l}</span></td>
      <td>
         <button className="bt bs bm" style={{padding:"2px 5px", marginRight:4}} onClick={()=>handleArchive(o.id)} title="В архив">📦</button>
         <button className="bt bs bm" style={{padding:"2px 5px", color:"var(--er)"}} onClick={()=>handleDelete(o.id)} title="Удалить">🗑</button>
      </td>
    </tr>)}</tbody></table></div>);
}
