  ...
  return(<><style>{CSS}</style><div className="R">
    ...
    <main className="M">
      ...
      <div className="CT">
        ...
        {pg==="analysis"&&<AnalysisPage {...{ord,sOrd,execs,scenarios,toast,getKey,gas:gas.current,gok}}/>}
...
// ═══ ANALYSIS PAGE ═══
function AnalysisPage({ord,sOrd,execs,scenarios,toast,getKey,gas,gok}){
  ...
  const runAnalysis=async()=>{if(!o)return;const key=getKey(o.aiProvider||"gemini");if(!key){toast("Нет ключа","error");return;}
    sRunning(true);sRunMsg("Чтение файлов с Drive...");sRunProg(10);
    try{
      let srcText=o.sourceText||"";
      // Read files from Drive 01_Originals
      if(gok&&gas){try{const driveFiles=await gas.gfb(o.id,"original");
        if(driveFiles?.files?.length){let ext="";
          for(const df of driveFiles.files){sRunMsg("Извлечение: "+df.fileName);
            try{if(df.mimeType?.startsWith("text/"))ext+=atob(df.base64)+"\n\n";
            else{ext+=(await callAI(key,o.aiModel||"gemini-2.0-flash","Извлеки ВЕСЬ текст. Только текст.",[{mime:df.mimeType,b64:df.base64}],o.aiProvider||"gemini"))+"\n\n";}}catch{}}
          if(ext.trim()){srcText=ext.trim().substring(0,50000);const u2={...o,sourceText:srcText.substring(0,5000),wordCount:srcText.split(/\s+/).length};sOrd(p=>p.map(x=>x.id===o.id?u2:x));}}
      }catch(e){toast("Drive: "+e.message,"error");}}
      sRunMsg("Анализ...");sRunProg(50);
      const t=await callAI(key,o.aiModel||"gemini-2.0-flash",(o.analysisPrompt||defAnalysisPrompt)+"\n\n"+srcText.substring(0,8000),null,o.aiProvider||"gemini");
      sRunProg(90);const u={...o,sourceText:srcText.substring(0,5000),wordCount:srcText.split(/\s+/).length,analysisResult:t};sOrd(p=>p.map(x=>x.id===o.id?u:x));toast("🔍 Анализ завершён","success");
  ...
  const runFullPipeline=async(order)=>{
    ...
    sRunning(true);sRunProg(5);sRunMsg("Извлечение текста...");
    let srcText=order.sourceText||"";
    try{
      // Step 1: Read original files from Drive and extract text via AI
      sRunMsg("Чтение файлов из Drive...");sRunProg(10);
      if(gok&&gas){try{
        const driveFiles=await gas.gfb(order.id,"original");
        if(driveFiles?.files?.length){sRunProg(15);
          let extracted="";
          for(let i=0;i<driveFiles.files.length;i++){
            const df=driveFiles.files[i];sRunMsg("Извлечение текста: "+df.fileName+"...");sRunProg(15+Math.round(i/driveFiles.files.length*15));
            try{
              if(df.mimeType?.startsWith("text/")){extracted+=atob(df.base64)+"\n\n";}
              else{const t=await callAI(key,model,"Извлеки ВЕСЬ текст из документа. Верни ТОЛЬКО текст.",[{mime:df.mimeType,b64:df.base64}],provider);extracted+=t+"\n\n";}
            }catch(e){toast(df.fileName+": ошибка","error");}
          }
          if(extracted.trim())srcText=extracted.trim().substring(0,50000);
        }
      }catch(e){toast("Drive: "+e.message,"error");}}
      if(!srcText)srcText=order.sourceText||"Файлы: "+order.fileName;
      // Step 2: Translate
      sRunMsg("Перевод...");sRunProg(30);
      ...
      sRunMsg("Сохранение перевода на Drive...");sRunProg(55);
      if(gok&&gas){try{const r=await gas.sf(order.id,"ai_translation",translation,order.langPair,order.id+"_AI_"+order.langPair+".txt");if(r?.fileUrl)fls.push({name:order.id+"_AI.txt",url:r.fileUrl,stage:"ai_translation"});}catch(e){toast("Drive перевод: "+e.message,"error");}}
      // Step 3: AI Editing
      let editedText=translation;let editorNotes="";
      if(order.useAiEdit){
        sRunMsg("Редактирование ИИ...");sRunProg(65);
        const editPrompt=(order.editPrompt||"Проверь перевод. Найди ошибки. Верни JSON: {correctedText:\"...\",errors:[{type,original,corrected,comment}]}")+"\n\nОригинал:\n"+srcText.substring(0,5000)+"\n\nПеревод:\n"+translation;
        const editResult=await callAI(key,model,editPrompt,null,provider);
        ...
        sRunMsg("Сохранение редактирования на Drive...");sRunProg(80);
      ...
      const updated={...order,stage:"review",sourceText:srcText.substring(0,5000),aiTranslation:translation,editedTranslation:editedText,editorNotes:editorNotes,fileLinks:fls,wordCount:srcText.split(/\s+/).length};
      sOrd(p=>p.map(x=>x.id===order.id?updated:x));
      if(gok&&gas){try{await gas.us(order.id,"На ревью");}catch{}}
      toast("✅ Перевод+Редактирование → Ревью","success");
...
function TranslPage({ord,sOrd,ak,aok,md,gas,gok,toast,getKey}){
  const items=ord.filter(o=>o.stage==="translate"||o.stage==="classify");
  const[running,sRunning]=useState(null);
  const run=async(o)=>{
    sRunning(o.id);
    const provider=o.aiProvider||"gemini";const model=o.aiModel||md;const key=getKey(provider);
    if(!key){toast("Нет ключа для "+provider,"error");sRunning(null);return;}
    ...
    // Read original from Drive if sourceText is empty
    let srcText=o.sourceText||"";
    if(!srcText&&gok&&gas){try{const df=await gas.gfb(o.id,"original");if(df?.files?.length){let ex="";for(const f of df.files){try{if(f.mimeType?.startsWith("text/"))ex+=atob(f.base64)+"\n\n";else ex+=(await callAI(key,model,"Извлеки ВЕСЬ текст.",[{mime:f.mimeType,b64:f.base64}],provider))+"\n\n";}catch{}}if(ex.trim()){srcText=ex.trim();const u2={...o,sourceText:srcText.substring(0,5000),wordCount:srcText.split(/\s+/).length};sOrd(p=>p.map(x=>x.id===o.id?u2:x));}}}catch{}}
    if(!srcText){toast("Нет текста для перевода","error");sRunning(null);return;}
    const prompt=(o.customPrompt||PR[o.docType]||"")+`\n${ln[lf]||lf}→${ln[lt]||lt}${gs}\nПереведи, ТОЛЬКО перевод:\n\n${srcText.substring(0,30000)}`;
    try{
      const t=await callAI(key,model,prompt,null,provider);
      if(!t){toast("Пустой ответ","error");sRunning(null);return;}
      const next=o.useAiEdit?"ai_edit":"review";
      const fls=[...(o.fileLinks||[])];
      // Save translation to Drive 02_AI_Translation
      if(gok&&gas){try{const r=await gas.sf(o.id,"ai_translation",t,o.langPair,o.id+"_AI_"+o.langPair+".txt");if(r?.fileUrl)fls.push({name:o.id+"_AI.txt",url:r.fileUrl,stage:"ai_translation"});await gas.us(o.id,next==="ai_edit"?"Ред. ИИ":"На ревью");}catch{}}
      const u={...o,stage:next,aiTranslation:t,editedTranslation:t,fileLinks:fls};
      sOrd(p=>p.map(x=>x.id===o.id?u:x));
      toast(`${o.id} → ${SM[next].l}`,"success");
    }catch(e){toast(e.message,"error");}sRunning(null);};

  if(!items.length)return<div className="em"><div className="ei">🤖</div><p>Нет заказов на перевод</p></div>;
  return(<>{items.map(o=><div key={o.id} className="cd">
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div>
      <div style={{fontSize:12,fontWeight:600}}>{o.id} — {o.clientName}</div>
      <div style={{fontSize:10,color:"var(--t3)"}}>{DT.find(d=>d.id===o.docType)?.icon} {o.langPair?.toUpperCase().replace("-"," → ")} · {o.wordCount} сл.{o.assignedTo&&" · 👤 "+o.assignedTo}</div>
      <div style={{fontSize:9,color:"var(--t3)",marginTop:2}}>ИИ: {AI_PROVIDERS.find(a=>a.id===(o.aiProvider||"gemini"))?.icon} {o.aiModel||md}</div>
    </div><button className="bt bv" disabled={running===o.id} onClick={()=>run(o)}>{running===o.id?<><span className="spn"/> Перевод...</>:"🤖 Перевести"}</button></div>
    {o.sourceText&&<div style={{marginTop:6,fontSize:10,color:"var(--t3)",maxHeight:60,overflow:"hidden"}}>{o.sourceText.substring(0,200)}...</div>}
  </div>)}</>);
}

// ═══ EDITOR PAGE ═══
function EditorPage({ord,sOrd,ak,aok,md,aep,gas,gok,toast,getKey}){
  const items=ord.filter(o=>o.stage==="ai_edit");
  ...
  const run=async(o)=>{
    ...
    if(!key){toast("Нет ключа для "+provider,"error");sRunning(null);return;}
    try{
      const p=customPrompt+"\n\nОригинал:\n"+(o.sourceText||"").substring(0,5000)+"\n\nЧерновой перевод:\n"+(o.aiTranslation||"");
      const t=await callAI(key,model,p,null,provider);
      ...
      const u={...o,stage:"review",editedTranslation:editedText,editorNotes:editorNotes};sOrd(pp=>pp.map(x=>x.id===o.id?u:x));
      if(gok&&gas){try{await gas.sf(o.id,"review",t,o.langPair,o.id+"_EDIT.txt");await gas.us(o.id,"На ревью","Ред. ИИ");}catch{}}
      toast(`${o.id} → Ревью`,"success");}
    }catch(e){toast(e.message,"error");}sRunning(null);};

  if(!items.length)return<div className="em"><div className="ei">🔧</div><p>Нет заказов на редактирование ИИ</p></div>;
  return(<><div className="cd"><div className="hd"><h3>🔧 Промпт редактора</h3></div>
    ...
    {items.map(o=><div key={o.id} className="cd">
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div>
        <div style={{fontSize:12,fontWeight:600}}>{o.id} — {o.clientName}</div>
        <div style={{fontSize:10,color:"var(--t3)"}}>{o.langPair?.toUpperCase().replace("-"," → ")} · {o.wordCount} сл.</div>
      </div><div className="bts" style={{marginTop:0}}><button className="bt bv" disabled={running===o.id} onClick={()=>run(o)}>{running===o.id?<><span className="spn"/></>:"🔧 Редактировать"}</button><button className="bt bs bm" onClick={()=>{sOrd(p=>p.map(x=>x.id===o.id?{...x,stage:"review"}:x));toast(o.id+" → Ревью","info")}}>⏭</button></div></div>
      {o.aiTranslation&&<div style={{marginTop:6,fontSize:10,color:"var(--t3)",maxHeight:60,overflow:"hidden"}}>{o.aiTranslation.substring(0,200)}...</div>}
    </div>)}</>);
}
...
function NewOrd({cur,sCur,ord,sOrd,ak,aok,md,gas,gok,aep,execs,toast,getKey,onGoAnalysis}){
  ...
  const launchWork=async()=>{sUpl(true);sPrg(5);
    ...
    const order={id:oid,clientName:cl||"Без имени",clientType:ct,fileName:df.map(f=>f.file.name).join(", ")||"Ручной",langPair:lp,docType:dt,complexity:cx,sourceText:srcT.substring(0,5000),stage:"classify",useAiEdit:ae,price:pr,notes:nt,createdAt:new Date().toISOString(),wordCount:wc,assignedTo:tr,aiProvider:"gemini",aiModel:md,fileLinks:fileLinks,aiTranslation:"",editedTranslation:"",editorNotes:""};
...
function RevPage({ord,sOrd,gas,gok,ak,aok,md,aep,toast}){
  ...
  if(a){const[f,t]=a.langPair.split("-");const fls=a.fileLinks||[];const origFiles=fls.filter(x=>x.stage==="original");const transFiles=fls.filter(x=>x.stage==="ai_translation"||x.stage==="final");
    ...
    {a.editorNotes&&<div className="ds wt">🔧 Заметки редактора: {a.editorNotes}</div>}
  ...
  return rv.map(o=><div key={o.id} className="cd" style={{cursor:"pointer"}} onClick={()=>{sAid(o.id);sTxt(o.editedTranslation||o.aiTranslation)}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><div style={{fontSize:12,fontWeight:600}}>✏️ {o.id} — {o.clientName}</div><div style={{fontSize:10,color:"var(--t3)"}}>{o.langPair?.toUpperCase().replace("-"," → ")} · {o.wordCount} сл.</div></div><button className="bt bp bm">Ревью →</button></div></div>);
}
