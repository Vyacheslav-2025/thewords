...
const SM={intake:{l:"Принят",i:"📥"},classify:{l:"Анализ",i:"🔍"},translate:{l:"AI Перевод",i:"🤖"},ai_edit:{l:"Ред. ИИ",i:"🔧"},review:{l:"Ревью",i:"✏️"},deliver:{l:"Готов",i:"✅"},archived:{l:"Архив",i:"📦"}};
...
class GAS{
  ...
  async get(p){const r=await fetch(this.u+"?"+new URLSearchParams(p),{redirect:"follow"});const d=await r.json();if(!d.success)throw new Error(d.error||"GAS error");return d.data||d}
  async post(body){const r=await fetch(this.u,{method:"POST",headers:{"Content-Type":"text/plain"},body:JSON.stringify(body),redirect:"follow"});const d=await r.json();if(!d.success)throw new Error(d.error||"GAS POST error");return d.data}
  ...
  del(id){return this.post({action:"deleteOrder",orderId:id})}
  go(){return this.get({action:"getOrders"})}
  gc(){return this.get({action:"getConfig"})}
  log(id,stage,err){return this.post({action:"logError",orderId:id||"",stage:stage||"",error:(err||"").substring(0,500)}).catch(()=>{})}
}
  ...
  return(<><style>{CSS}</style><div className="R">
    ...
    <main className="M">
      ...
      <div className="CT">
        ...
        {pg==="ord"&&<OrdPage {...{ord,sOrd,gas:gas.current,gok,toast}}/>}
...
function NewOrd({cur,sCur,ord,sOrd,ak,aok,md,gas,gok,execs,toast,getKey,fm,driveOk,onGoAnalysis}){
  ...
  const addSS=async(files)=>{const a=[];for(const f of files){try{const b64=await f2b(f);a.push({file:f,b64,mime:getMime(f),prev:URL.createObjectURL(f)});}catch(e){toast("Ошибка чтения: "+f.name,"error");}}sSS(p=>[...p,...a]);};
...
function AnalysisPage({ord,sOrd,execs,scenarios,toast,getKey,gas,gok,fm,driveOk,md,onGoPage}){
  ...
  if(o) return(<><button className="bt bs bm" onClick={()=>sSel(null)} style={{marginBottom:8}}>← Назад</button>
    ...
    <div style={{display:"flex",alignItems:"center",gap:0,marginBottom:10,flexWrap:"wrap"}}>
      {["classify","translate","ai_edit","review","deliver"].map((st,i,arr)=>{
        const cur=st===o.stage;const past=arr.indexOf(o.stage)>i;
        return<div key={st} style={{display:"flex",alignItems:"center"}}><button className={`bt bm ${cur?"bp":past?"bo":"bs"}`} onClick={()=>{upd("stage",st);if(gok2&&gas2)gas2.us(o.id,SM[st]?.l||st).catch(()=>{});toast(SM[st]?.l,"info");}} style={{fontSize:9,minWidth:0}}>{SM[st]?.i} {SM[st]?.l}</button>{i<arr.length-1&&<span style={{color:"var(--t3)",margin:"0 2px",fontSize:10}}>→</span>}</div>})}
    </div>
  ...
  const[sort,sSort]=useState("new");
  if(!active.length)return<div className="em"><div className="ei">🔍</div><p>Нет активных заказов</p></div>;
  return(<><SortBar sort={sort} sSort={sSort}/>{sortOrders(active,sort).map(o=><div key={o.id} className="cd" style={{cursor:"pointer"}} onClick={()=>sSel(o.id)}>
...
function TranslPage({ord,sOrd,toast,fm,driveOk}){
  const[sort,sSort]=useState("new");
  const withTranslation=sortOrders(ord.filter(o=>o.aiTranslation),sort);
  if(!withTranslation.length)return<div className="em"><div className="ei">🤖</div><p>Нет переводов</p></div>;
  return(<><SortBar sort={sort} sSort={sSort}/>{withTranslation.map(o=>{const tFiles=(o.fileLinks||[]).filter(f=>f.stage==="ai_translation");
...
function EditorPage({ord,sOrd,toast,fm,driveOk}){
  const[sort,sSort]=useState("new");
  const edited=sortOrders(ord.filter(o=>o.editedTranslation&&o.editedTranslation!==o.aiTranslation||o.editorNotes),sort);
  if(!edited.length)return<div className="em"><div className="ei">🔧</div><p>Нет редактированных заказов</p></div>;
  return(<><SortBar sort={sort} sSort={sSort}/>{edited.map(o=>{const eFiles=(o.fileLinks||[]).filter(f=>f.stage==="editing"||f.stage==="errors");
    return<div key={o.id} className="cd"><div style={{fontSize:12,fontWeight:600}}>{o.id} — {o.clientName}</div>
...
// ═══ ORDERS ═══
function SortBar({sort,sSort}){
  return<div style={{display:"flex",gap:4,marginBottom:8,flexWrap:"wrap"}}>
    {[{id:"new",l:"Новые ↓"},{id:"old",l:"Старые ↑"},{id:"name",l:"Имя А-Я"},{id:"name_r",l:"Имя Я-А"}].map(s=>
      <button key={s.id} className={`bt bm ${sort===s.id?"bp":"bs"}`} onClick={()=>sSort(s.id)}>{s.l}</button>)}
  </div>;
}
function sortOrders(list,sort){
  const a=[...list];
  if(sort==="new")return a.reverse();
  if(sort==="old")return a;
  if(sort==="name")return a.sort((x,y)=>(x.clientName||"").localeCompare(y.clientName||""));
  if(sort==="name_r")return a.sort((x,y)=>(y.clientName||"").localeCompare(x.clientName||""));
  return a.reverse();
}

function OrdPage({ord,sOrd,gas,gok,toast}){
  const[sort,sSort]=useState("new");
  const[cfm,sCfm]=useState(null);
  const sorted=sortOrders(ord,sort);
  const del=async(o)=>{if(gok&&gas){try{await gas.del(o.id);toast("Sheets: удалён","success");}catch(e){toast("Sheets: "+e.message,"error");}}sOrd(p=>p.filter(x=>x.id!==o.id));sCfm(null);};
  const archive=async(o)=>{sOrd(p=>p.map(x=>x.id===o.id?{...x,stage:"archived"}:x));if(gok&&gas){try{await gas.us(o.id,"Архив");}catch{}}toast(o.id+" → Архив","info");};
  if(!ord.length)return<div className="em"><div className="ei">📋</div><p>Пусто</p></div>;
  return(<><SortBar sort={sort} sSort={sSort}/>
    <div className="cd" style={{padding:0,overflow:"auto"}}><table className="ot"><thead><tr><th>ID</th><th>Клиент</th><th>Языки</th><th>Тип</th><th>₸</th><th>Статус</th><th></th></tr></thead>
    <tbody>{sorted.filter(o=>o.stage!=="archived").map(o=><tr key={o.id}><td style={{fontFamily:"'JetBrains Mono',monospace",color:"var(--ac)"}}>{o.id}</td><td>{o.clientName}</td><td>{o.langPair?.toUpperCase().replace("-"," → ")}</td><td>{DT.find(d=>d.id===o.docType)?.label||""}</td><td style={{fontSize:9}}>{o.price?o.price+"₸":"—"}</td><td><span className={`stg ${o.stage==="deliver"?"sdl":o.stage==="review"?"srv":"si"}`}>{SM[o.stage]?.l}</span></td>
    <td style={{whiteSpace:"nowrap"}}>{cfm===o.id?<><button className="bt bm" style={{background:"var(--er)",color:"#fff"}} onClick={()=>del(o)}>Да</button><button className="bt bs bm" onClick={()=>sCfm(null)}>✕</button></>:<><button className="bt bs bm" title="Архив" onClick={()=>archive(o)}>📦</button><button className="bt bs bm" title="Удалить" style={{color:"var(--er)"}} onClick={()=>sCfm(o.id)}>🗑</button></>}</td></tr>)}</tbody></table></div>
    {sorted.some(o=>o.stage==="archived")&&<div className="cd" style={{marginTop:10}}><div className="hd"><h3>📦 Архив</h3></div>
    {sorted.filter(o=>o.stage==="archived").map(o=><div key={o.id} style={{fontSize:10,padding:"3px 0",display:"flex",justifyContent:"space-between"}}><span>{o.id} — {o.clientName}</span><button className="bt bs bm" onClick={()=>{sOrd(p=>p.map(x=>x.id===o.id?{...x,stage:"classify"}:x));toast("Восстановлен","info")}}>↩️</button></div>)}</div>}
  </>);
}
...
function RevPage({ord,sOrd,gas,gok,toast,fm,driveOk}){
  ...
  const[sort,sSort]=useState("new");const sorted=sortOrders(rv,sort);
  return<><SortBar sort={sort} sSort={sSort}/>{sorted.map(o=><div key={o.id} className="cd" style={{cursor:"pointer"}} onClick={()=>sAid(o.id)}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><div style={{fontSize:12,fontWeight:600}}>✏️ {o.id} — {o.clientName}</div><div style={{fontSize:10,color:"var(--t3)"}}>{o.langPair?.toUpperCase().replace("-"," → ")} · {SM[o.stage]?.l}</div></div><button className="bt bp bm">Ревью →</button></div>
    {(o.fileLinks||[]).filter(f=>f.stage==="original").map((f,i)=><div key={"o"+i} className="flink"><a href={f.url} target="_blank" rel="noopener">📄 {f.name}</a></div>)}
    {(o.fileLinks||[]).filter(f=>f.stage==="ai_translation").map((f,i)=><div key={"t"+i} className="flink"><a href={f.url} target="_blank" rel="noopener">🤖 {f.name}</a></div>)}
    {(o.fileLinks||[]).filter(f=>f.stage==="errors").map((f,i)=><div key={"e"+i} className="flink"><a href={f.url} target="_blank" rel="noopener" style={{color:"var(--or)"}}>📋 {f.name}</a></div>)}
  </div>)}</>;
}
