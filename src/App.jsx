...
class GAS{
  ...
  async uf(id,st,b64,mime,fn){
    // Try POST first, fallback to GET for small files
    try{
      const r=await fetch(this.u,{method:"POST",body:JSON.stringify({action:"uploadBase64File",orderId:id,stage:st,base64Data:b64,mimeType:mime,fileName:fn}),redirect:"follow"});
      const t=await r.text();try{const d=JSON.parse(t);if(d.success)return d.data;throw new Error(d.error||"POST failed")}catch{throw new Error("POST parse error: "+t.substring(0,100))}
    }catch(e){
      // Fallback: if file is small enough, try GET
      if(b64.length<7000){return this.c({action:"uploadBase64File",orderId:id,stage:st,base64Data:b64,mimeType:mime,fileName:fn})}
      // For large files, save as text content via existing saveFile
      return this.c({action:"saveFile",orderId:id,stage:st,content:"[BASE64_FILE:"+fn+":"+mime+":"+b64.substring(0,100)+"...]",langPair:"",fileName:fn})
    }
  }
...
function AnalysisPage({ord,sOrd,execs,scenarios,toast,getKey,gas,gok,onGoPage}){
  ...
  const runFullPipeline=async(order)=>{
    ...
    const defProvider=order.aiProvider||"gemini";const defModel=order.aiModel||"gemini-2.0-flash";
    ...
    try{
      ...
      const trProvider=order.aiTranslateProvider||defProvider;const trModel=order.aiTranslateModel||defModel;const trKey=getKey(trProvider);if(!trKey){toast("Нет ключа для перевода","error");sRunning(false);return;}
      sRunMsg("Перевод "+fileImgs.length+" файл(ов) ("+trProvider+")...");sRunProg(20);
      ...
      const translation=await callAI(trKey,trModel,trPrompt,fileImgs.map(f=>({mime:f.mime,b64:f.b64})),trProvider);
      ...
      if(order.useAiEdit){
        const edProvider=order.aiEditProvider||defProvider;const edModel=order.aiEditModel||defModel;const edKey=getKey(edProvider);
        sRunMsg("Редактирование ("+edProvider+")...");sRunProg(65);
        ...
        const editResult=await callAI(edKey||trKey,edModel,editPrompt,fileImgs.map(f=>({mime:f.mime,b64:f.b64})),edProvider);
  ...
  if(o) return(<><button className="bt bs bm" onClick={()=>sSel(null)} style={{marginBottom:8}}>← Назад</button>
    ...
    <div className="cd"><div className="hd"><h3>2️⃣ Промпт перевода</h3><span className="bg">Перевод</span></div>
      ...
      <div className="fg" style={{marginTop:6}}><label className="fl">ИИ для перевода</label><div>{AI_PROVIDERS.map(ai=><span key={ai.id} className={`ai-chip ${(o.aiTranslateProvider||o.aiProvider||"gemini")===ai.id?"on":""}`} onClick={()=>{const u={...o,aiTranslateProvider:ai.id};sOrd(p=>p.map(x=>x.id===o.id?u:x))}}>{ai.icon} {ai.name}{getKey(ai.id)?"":" ⚠"}</span>)}</div></div>
      <div className="fg"><label className="fl">Модель перевода</label><select className="fsl" value={o.aiTranslateModel||o.aiModel||""} onChange={e=>{const u={...o,aiTranslateModel:e.target.value};sOrd(p=>p.map(x=>x.id===o.id?u:x))}}>{(AI_PROVIDERS.find(a=>a.id===(o.aiTranslateProvider||o.aiProvider||"gemini"))?.models||[]).map(m=><option key={m} value={m}>{m}</option>)}</select></div>
    </div>
    <div className="cd"><div className="hd"><h3>3️⃣ Промпт редактирования</h3><span className="bg">Редактор</span></div>
      ...
      <div className="fg" style={{marginTop:6}}><label className="fl">ИИ для редактирования</label><div>{AI_PROVIDERS.map(ai=><span key={ai.id} className={`ai-chip ${(o.aiEditProvider||o.aiProvider||"gemini")===ai.id?"on":""}`} onClick={()=>{const u={...o,aiEditProvider:ai.id};sOrd(p=>p.map(x=>x.id===o.id?u:x))}}>{ai.icon} {ai.name}{getKey(ai.id)?"":" ⚠"}</span>)}</div></div>
      <div className="fg"><label className="fl">Модель редактирования</label><select className="fsl" value={o.aiEditModel||o.aiModel||""} onChange={e=>{const u={...o,aiEditModel:e.target.value};sOrd(p=>p.map(x=>x.id===o.id?u:x))}}>{(AI_PROVIDERS.find(a=>a.id===(o.aiEditProvider||o.aiProvider||"gemini"))?.models||[]).map(m=><option key={m} value={m}>{m}</option>)}</select></div>
    </div>
  ...
  return(<>
    ...
    {edited.map(o=>{const eFiles=(o.fileLinks||[]).filter(f=>f.stage==="editing");
      return<div key={o.id} className="cd"><div style={{fontSize:12,fontWeight:600}}>{o.id} — {o.clientName}</div>
        ...
        {(o.editorNotes||(o.fileLinks||[]).some(f=>f.stage==="editing"))&&<div style={{marginTop:8,padding:8,background:"#F5943A12",border:"1px solid #F5943A30",borderRadius:5}}><div style={{fontSize:11,fontWeight:600,color:"var(--or)",marginBottom:4}}>📋 Отчёт об ошибках</div>{(o.fileLinks||[]).filter(f=>f.stage==="editing").map((f,i)=><div key={i} style={{padding:"2px 0"}}><a href={f.url} target="_blank" rel="noopener" style={{fontSize:11,color:"var(--ac)"}}>📄 {f.name} (Drive)</a></div>)}{!(o.fileLinks||[]).some(f=>f.stage==="editing")&&<div style={{fontSize:10,color:"var(--t3)"}}>Отчёт доступен после сохранения на Drive</div>}</div>}
      </div>})}</>}
...
function NewOrd({cur,sCur,ord,sOrd,ak,aok,md,gas,gok,aep,execs,toast,getKey,onGoAnalysis}){
  ...
  const[ss,sSS]=useState([]);const[selExec,sSelExec]=useState("");const[df,sDF]=useState([]);const[st,sSt]=useState("");const[mn,sMn]=useState("");
  ...
  const launchWork=async()=>{sUpl(true);sPrg(5);
    ...
    for(let i=0;i<df.length;i++){const doc=df[i];sPrg(5+Math.round(i/df.length*70));
      if(gok&&gas){try{
        if(doc.b64){const r=await gas.uf(oid,"original",doc.b64,doc.mime,doc.file.name);fileLinks.push({name:doc.file.name,url:r?.viewUrl||r?.fileUrl||"",stage:"original"});toast("📁 "+doc.file.name+" → Drive","success");}
        else if(doc.text){const r=await gas.sf(oid,"original",doc.text,lp,doc.file.name);fileLinks.push({name:doc.file.name,url:r?.fileUrl||"",stage:"original"});toast("📁 "+doc.file.name+" → Drive","success");}
      }catch(e){toast("Upload "+doc.file.name+": "+e.message,"error");fileLinks.push({name:doc.file.name,url:"",stage:"original"});}}
      else{fileLinks.push({name:doc.file.name,url:"",stage:"original"});}}
    sPrg(80);
  ...
  if(!res) return(<>
    ...
    <div className="cd"><div className="hd"><h3>📥 Данные</h3><span className="bg">Шаг 1</span></div>
      ...
      <div className="fg"><label className="fl">Исполнитель</label><select className="fsl" value={selExec} onChange={e=>sSelExec(e.target.value)}><option value="">— Авто —</option>{execs.map(ex=><option key={ex.id} value={ex.name}>{ex.name} ({ex.role==="translator"?"Перев.":ex.role==="editor"?"Ред.":"Оба"})</option>)}</select></div>
...
function RevPage({ord,sOrd,gas,gok,ak,aok,md,aep,toast}){
  ...
  if(a){const[f,t]=a.langPair.split("-");const fls=a.fileLinks||[];const origFiles=fls.filter(x=>x.stage==="original");const transFiles=fls.filter(x=>x.stage==="ai_translation"||x.stage==="final");
    ...
    {(a.editorNotes||(a.fileLinks||[]).length>0)&&<div style={{padding:10,background:"var(--b1)",border:"1px solid var(--bd)",borderRadius:5,marginBottom:10}}><div style={{fontSize:12,fontWeight:600,marginBottom:6}}>📁 Файлы на Drive</div>{(a.fileLinks||[]).filter(f=>f.stage==="original").map((f,i)=><div key={i} style={{padding:"2px 0"}}><a href={f.url} target="_blank" rel="noopener" style={{fontSize:11}}>📄 {f.name} (Оригинал)</a></div>)}{(a.fileLinks||[]).filter(f=>f.stage==="ai_translation").map((f,i)=><div key={i} style={{padding:"2px 0"}}><a href={f.url} target="_blank" rel="noopener" style={{fontSize:11,color:"var(--in)"}}>🤖 {f.name} (Перевод)</a></div>)}{(a.fileLinks||[]).filter(f=>f.stage==="editing").map((f,i)=><div key={i} style={{padding:"2px 0"}}><a href={f.url} target="_blank" rel="noopener" style={{fontSize:11,color:"var(--or)"}}>📋 {f.name} (Отчёт об ошибках)</a></div>)}</div>}
...
function ScenarioSettings({scenarios,sScenarios,toast}){
  ...
  const[steps,sSteps]=useState([]);
  const[cxIds,sCxIds]=useState([]);
  ...
  const addStep=()=>sSteps(p=>[...p,{stage:"translate",ai:"gemini",model:"gemini-2.0-flash"}]);
  const rmStep=i=>sSteps(p=>p.filter((_,j)=>j!==i));
  const updStep=(i,k,v)=>sSteps(p=>p.map((s,j)=>j===i?{...s,[k]:v}:s));
  const toggleCx=id=>sCxIds(p=>p.includes(id)?p.filter(x=>x!==id):[...p,id]);
  ...
  if(ed!==null)return(<div className="SC SF"><h4>📋 {ed==="new"?"Новый сценарий":"Редактирование"}</h4>
    <div className="fg"><label className="fl">Название</label><input className="fi" value={nm} onChange={e=>sNm(e.target.value)} placeholder="Напр: Юрлицо стандарт"/></div>
    <div className="fg"><label className="fl">Этапы работы</label>
      {steps.map((s,i)=><div key={i} className="step-card"><div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}><span className="step-num">{i+1}</span>
        <select className="fsl" style={{flex:1}} value={s.stage} onChange={e=>updStep(i,"stage",e.target.value)}>{allStages.map(st=><option key={st} value={st}>{SM[st]?.i} {SM[st]?.l}</option>)}<option value="human_review">👤 Ручное ревью</option></select>
        <button className="bt bs bm" style={{color:"var(--er)"}} onClick={()=>rmStep(i)}>✕</button></div>
        <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>{AI_PROVIDERS.map(ai=><span key={ai.id} className={`ai-chip ${s.ai===ai.id?"on":""}`} onClick={()=>updStep(i,"ai",ai.id)}>{ai.icon} {ai.name}</span>)}<span className={`ai-chip ${s.ai==="human"?"on":""}`} onClick={()=>updStep(i,"ai","human")}>👤 Человек</span></div>
        {s.ai!=="human"&&<select className="fsl" style={{marginTop:4}} value={s.model||""} onChange={e=>updStep(i,"model",e.target.value)}>{(AI_PROVIDERS.find(a=>a.id===s.ai)?.models||[]).map(m=><option key={m} value={m}>{m}</option>)}</select>}
      </div>)}
      <button className="bt bs bm" onClick={addStep} style={{marginTop:4}}>➕ Добавить этап</button>
    </div>
    <div className="fg"><label className="fl">Применять к сложности</label><div className="cxg">{CX.map(c=><button key={c.id} className={`cxc ${cxIds.includes(c.id)?"sl":""}`} onClick={()=>toggleCx(c.id)}>{c.label}</button>)}</div><div style={{fontSize:8,color:"var(--t3)",marginTop:2}}>Пустой = для всех</div></div>
    <div className="bts"><button className="bt bp" onClick={save}>💾 Сохранить</button><button className="bt bs" onClick={()=>sEd(null)}>Отмена</button></div>
  </div>);

  return(<div className="SC SF"><h4>📋 Сценарии перевода</h4>
    <div className="bts" style={{marginTop:0,marginBottom:8}}><button className="bt bp bm" onClick={startNew}>➕ Новый сценарий</button></div>
    {scenarios.map(sc=><div key={sc.id} className="scenario-card" onClick={()=>startEdit(sc)}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div>
        ...
        <div style={{fontSize:9,color:"var(--t3)",marginTop:2}}>{(sc.steps||[]).map((s,j)=><span key={j}>{j>0?" → ":""}{SM[s.stage]?.i||"👤"} {s.ai==="human"?"👤":(AI_PROVIDERS.find(a=>a.id===s.ai)?.icon||"")} {SM[s.stage]?.l||s.stage}</span>)}</div>
        {sc.complexities?.length>0&&<div style={{marginTop:3}}>{sc.complexities.map(c=><span key={c} className="exec-tag">{CX.find(x=>x.id===c)?.s||c}</span>)}</div>}
      </div><button className="bt bs bm" onClick={e=>{e.stopPropagation();sScenarios(p=>p.filter(s=>s.id!==sc.id));toast("Удалён","info")}} style={{color:"var(--er)"}}>🗑</button></div>
    </div>)}
  </div>);
}
