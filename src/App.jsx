  ...
  return(<>
    ...
    {edited.map(o=>{const eFiles=(o.fileLinks||[]).filter(f=>f.stage==="editing");
      return<div key={o.id} className="cd"><div style={{fontSize:12,fontWeight:600}}>{o.id} — {o.clientName}</div>
        ...
        {o.editorNotes&&<div style={{marginTop:8,padding:10,background:"#F5943A12",border:"1px solid #F5943A30",borderRadius:5}}><div style={{fontSize:12,fontWeight:600,color:"var(--or)",marginBottom:6}}>📋 Отчёт об ошибках</div><div style={{fontSize:11,whiteSpace:"pre-wrap",color:"var(--t1)",lineHeight:1.6}}>{o.editorNotes}</div>{(o.fileLinks||[]).filter(f=>f.name?.includes("ERROR")||f.name?.includes("NOTES")).map((f,i)=><div key={i} style={{marginTop:6}}><a href={f.url} target="_blank" rel="noopener" style={{fontSize:11,color:"var(--ac)"}}>📄 {f.name} (Google Drive)</a></div>)}</div>}
      </div>})}</>}
...
function RevPage({ord,sOrd,gas,gok,ak,aok,md,aep,toast}){
  ...
  if(a){const[f,t]=a.langPair.split("-");const fls=a.fileLinks||[];const origFiles=fls.filter(x=>x.stage==="original");const transFiles=fls.filter(x=>x.stage==="ai_translation"||x.stage==="final");
    ...
    {a.editorNotes&&<div style={{padding:10,background:"#F5943A12",border:"1px solid #F5943A30",borderRadius:5,marginBottom:10}}><div style={{fontSize:12,fontWeight:700,color:"var(--or)",marginBottom:6}}>📋 Отчёт об ошибках (для переводчика)</div><div style={{fontSize:11,whiteSpace:"pre-wrap",color:"var(--t1)",lineHeight:1.6}}>{a.editorNotes}</div>{(a.fileLinks||[]).filter(f=>f.stage==="editing").map((f,i)=><div key={i} style={{marginTop:6}}><a href={f.url} target="_blank" rel="noopener" style={{fontSize:11,color:"var(--ac)"}}>📄 {f.name} (Drive)</a></div>)}{(a.fileLinks||[]).filter(f=>f.stage==="ai_translation").map((f,i)=><div key={i} style={{marginTop:4}}><a href={f.url} target="_blank" rel="noopener" style={{fontSize:11,color:"var(--in)"}}>🤖 {f.name} (Перевод — Drive)</a></div>)}</div>}
